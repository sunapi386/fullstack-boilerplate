...
export class UserResolver {
  ...
  async forgotPassword(@Arg("email") email: string): Promise<boolean> {
    try {
      ...
      if (user) {
        ...
        const token = createToken({ id: user.id, preResetPasswordHash: user.password }, {expiresIn: "1d"})
  ...
  async resetPassword(@Arg("data") data: ResetPasswordInput): Promise<boolean> {
    const payload = decryptToken<{ id: string, preResetPasswordHash: string }>(data.token)
    await this.userService.resetPassword(payload.id, payload.preResetPasswordHash, data.password)
    return true
  }
}
